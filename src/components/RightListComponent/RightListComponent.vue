...
<template>
    <div class="component-item-right">
        <div class="right-dolist">
            <i
                title="查看组件Widget属性"
                class="uer uer-info-circle"
                @click.stop="getWidgetInfo(currentItem)"
            ></i>
            <i
                :title="currentItem.isLock ? lockStatus[1]:lockStatus[0]"
                :class="{'uer': true, 'uer-lock': !currentItem.isLock, 'uer-unlock': currentItem.isLock}"
                @click.stop="setLock(currentItem)"
            ></i>
            <!-- <i
                            v-show="currentItem.codePosition"
                            class="uer uer-code1"
                            @click.stop="openCodeWin(currentItem)"
            ></i>-->
            <i title="删除" class="uer uer-close-circle" @click.stop="deleteComponent(currentItem)"></i>
            <i
                ref="moreIcon"
                title="更多"
                class="uer uer-ellipsis"
                @mouseenter.stop="showMorePanel($event, true, currentItem.id)"
                @mouseleave.stop="showMorePanel($event, false, currentItem.id)"
            >
                <div
                    ref="morePanel"
                    class="ue-more-panel"
